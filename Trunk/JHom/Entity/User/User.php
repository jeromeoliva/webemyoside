...
class User extends JHomEntity
{
	...
	function User($core)
	{
		...
		$this->Groupe=new EntityProperty("Group","GroupeId");
	...
	function Verify()
	{
		...
		$User->AddArgument(new Argument("User","Email",EQUAL,JVar::GetPost("sourceControl")));
		$Users = $User->GetByArg();
   ...
   */
	...
	function GetImageMini()
	{
		if($this->Image->Value)
		{
			...
			return JFile::GetMini($this->Image->Value)."?".rand(0, 500);
		}
		else
		{
    ...
    static function ClearDateConnect($core)
    {
   	 ...
   	 *  */
		...
		if(sizeof($files)> 0)
		{
			...
			{
				JFile::Delete($file["Name"], "../ImagesUser/User/User".$userId);
			}
		}

		//Suppression du repertoire
		JFile::DeleteDirectory("../ImagesUser/User/User".$userId);

		//Suppression de la photo
		JFile::Delete(md5("User".$userId).".jpeg","../ImagesUser/User");
	  ...
	  * */
   	 public function SearchUser($name ='', $firstName ='', $email ='')
   	 {
   	 	...
   	 	{
			$request = "  SELECT Id FROM `ee_user` WHERE (Name like '".JFormat::EscapeString(JVar::GetPost("sourceControl"))."%'";
			$request .= " OR FirstName like '".JFormat::EscapeString(JVar::GetPost("sourceControl"))."%' )";
			$request .= " AND Id <>".$this->Core->User->IdEntite;
	   	}
		...
		if(sizeof($results) > 0)
		{
			...
			$btnSend->AddStyle('width','150px');
                        ...
                        if(JVar::GetPost("AddAction"))
                        {
                		$btnSend->OnClick = JVar::GetPost("AddAction");
		        }
 ...
 * */
 class UserGroup extends JHomEntity
 {
 	protected $User;
 ...
 * Membre des groupe
 * */
class UserGroupUser extends JHomEntity
 {
 	protected $User;
 	protected $Group;
