...
class TabStrip extends JHomControl
{
	//Propriete
	...
	//Constructeur
	function TabStrip($name="", $app, $widget="")
	{
		//Version
	...
	function Show()
	{
		...
		{
			if($iTab == $Selected)
				$TextControl .="<div id='tab_".$iTab."'  >".$tab->Show()."</div>";
			else
				$TextControl .="<div id='tab_".$iTab."'  style='height:0px;overflow:hidden;display: none;'>".$tab->Show()."</div>";
			$iTab++;
		}
		$TextControl .="</div>";
		return $TextControl;
