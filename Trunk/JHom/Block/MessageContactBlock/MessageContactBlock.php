...
class MessageContactBlock extends JHomBlock implements IJhomBlock
{
	// Propriété
	private $EditMode;

	//Constructeur
	function MessageContactBlock($core="", $editMode=SIMPLE)
	{
		//Version
		...
		//Champs de saisie
		$this->tbName = new BsTextBox("tbName");
		$this->tbName->Title = $this->Core->GetCode("YourName");
		...
		$this->tbEmail = new BsEmailBox("tbEmail");
		$this->tbEmail->Title = $this->Core->GetCode("YourEmail");
		...
		$this->btnSend->CssClass = "btn btn-primary";
	...
	function SendMessage()
	{
		...
		if(	JVar::GetPost("tbName") != ""
			/*&& JVar::GetPost("tbFirstName") != ""*/
			&& JVar::GetPost("tbEmail") != ""
			&&  $this->tbEmail->Verify()
		)
		{
			$Email  = new JEmail();
			...
			{
				$Email->Body = JVar::GetPost("tbName")." " ;
				...
				$Email->Body .= JVar::GetPost("tbMessage");
			}
			else
			{
				$Email->Body = JVar::GetPost("tbFirstName")." pour la société " ;
				$Email->Body .= JVar::GetPost("tbFirstName");
				$Email->Body .= "<br/> a envoyé un message ";

				switch(JVar::GetPost("rbNature"))
				{
					case 0:
				...
				$Email->Body .= "<br/> Son email est  : ".JVar::GetPost("tbEmail");
				$Email->Body .= "<br/> Son télephone est  : ".JVar::GetPost("tbPhone");
				$Email->Body .= "<br/> Son fonction est  : ".JVar::GetPost("tbFonction");
				$Email->Body .= "<br/> Voici son message :";
